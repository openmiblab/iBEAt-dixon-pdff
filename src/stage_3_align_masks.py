...
def run_site(build, group, site=None):
    ...
    for fixed_series_water in tqdm(all_water_series, desc='Aligning dixons'):
        ...
        coreg_series_water = coreg_study + [(f'{sequence}_water', 0)]
        ...
        try:
            ...
            # Read the fixed water and pdffvolume
            fixed_vol = db.volume(fixed_series_water)
            fixed_values_pdff = db.volume(fixed_series_pdff).values

            # Read the other volumes and reslice (in case geometries are not matched)
            moving_vol_water = db.volume(moving_series_water).slice_like(fixed_vol) # TODO: remove slice_like
            moving_vol_kidney_masks = db.volume(moving_series_kidney_masks)#.slice_like(fixed_vol)
            moving_vol_total_mr = db.volume(moving_series_total_mr)#.slice_like(fixed_vol)
            moving_vol_tissue_types_mr = db.volume(moving_series_tissue_types_mr)#.slice_like(fixed_vol)
            ...
            coreg_values_kidney_masks = transform_label(moving_vol_kidney_masks.values, transfo, fixed_values_pdff, 'total_mr', spacing)
            coreg_values_total_mr = transform_label(moving_vol_total_mr.values, transfo, fixed_values_pdff, 'total_mr', spacing)
            coreg_values_tissue_types_mr = transform_label(moving_vol_tissue_types_mr.values, transfo, fixed_values_pdff, 'tissue_types_mr', spacing)
            
            # Save results 
            db.write_volume((coreg_values_water, fixed_vol.affine), coreg_series_water, ref=moving_series_water)
            db.write_volume((coreg_values_kidney_masks, fixed_vol.affine), coreg_series_kidney_masks, ref=moving_series_kidney_masks)
            db.write_volume((coreg_values_total_mr, fixed_vol.affine), coreg_series_total_mr, ref=moving_series_total_mr)
            db.write_volume((coreg_values_tissue_types_mr, fixed_vol.affine), coreg_series_tissue_types_mr, ref=moving_series_tissue_types_mr)
...
def transform_label(label, transfo, pdff, task, spacing):
    scl = 1000
    values = np.unique(label)
    transformed = np.zeros_like(label)
    for v in values:

        # Transform and turn into binary
        mask = (label == v).astype(np.float32)
        mask_transformed = transform(scl * mask, transfo, spacing=spacing)
        ...
        if task=='total_mr':
            ...
            if v in [1,2]: # visceral and subcutaneous fat
                min_pdff = 0.0
                max_pdff = np.inf
        mask_transformed[pdff < min_pdff] = 0
        mask_transformed[pdff > max_pdff] = 0
...
if __name__=='__main__':
    ...
    logging.basicConfig(
        filename=os.path.join(dixon_build, 'stage_3_align_masks.log'),
        level=logging.INFO,
